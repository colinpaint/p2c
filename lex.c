  ...
  switch (inbufptr[2]) {
    ...
    case 'C':
      if (toupper(inbufptr[1]) == 'S' &&
          (inbufptr[3] == '+' || inbufptr[3] == '-') && inbufptr + 4 == closing) {
        if (shortcircuit < 0)
    ...
    case 'F':
      if (toupper(inbufptr[1]) == 'I' &&
          ((toupper(inbufptr[3]) == 'O' && toupper(inbufptr[4]) == 'P' && toupper(inbufptr[5]) == 'T') ||
           (toupper(inbufptr[3]) == 'D' && toupper(inbufptr[4]) == 'E' && toupper(inbufptr[5]) == 'F') ||
             (toupper(inbufptr[3]) == 'N' && toupper(inbufptr[4]) == 'D' &&
              toupper(inbufptr[5]) == 'E' && toupper(inbufptr[6]) == 'F'))) {
  ...
  *cp2 = 0;
  ...
  if (chgmode == 'T' && lex_initialized) {
    if (cp == closing || *cp == '=' || *cp == '+' || *cp == '-')
      warning(format_s("%s works only at top of program [235]", rctable[i].name));
    }
  ...
  switch (kind) {
    ...
    case 'R':
      ...
      if (toupper(cp[0]) == 'D' &&
        ...
        cp2 = cp;
        while (isdigit(*cp) || *cp == '-' || *cp == '+' || *cp == '.' || toupper(*cp) == 'E')
          cp++;
        if (cp == cp2)
          return 0;
        dval = atof(cp2);
    ...
    case 'B':
      ...
      strlist_insert(&rcprevvalues[i], "")->value = *((short *)rctable[i].ptr);
...
// 0={ }, 1=(* *), 2=C comments, 3=" "
  ...
  for (;;) {
    ...
    switch (ch) {
      ...
      //{{{
      case '*':
        if (*inbufptr == ((starparen == 2) ? '/' : ')') && starparen < 3 &&
            (starparen || nestedcomments == 0) && --nestcount <= 0) {
          inbufptr++;
          *cp = 0;
      ...
      //{{{
      case '(':
        if (*inbufptr == '*' && nestedcomments == 1 && starparen < 2) {
          *cp++ = ch;
          ch = *inbufptr++;
          nestcount++;
          }
      ...
      case '$':
        ...
        do {
          ...
          if (curtok == TOK_IF) { /* $IF expr$ */
            ...
            if (!strcmp(curtokbuf, "INCLUDE") && !if_flag && !skipping_module) {
              //{{{  include
              char *fn;
              gettok();
              if (curtok == TOK_IDENT) {
      ...
      default:
        if (isdigit (ch)) {
          ...
          if (i != 10) {
            //{{{  dec ??
            curtokint = 0;
            while (isdigit(*inbufptr) || (i > 10 && isxdigit(*inbufptr))) {
              ch = toupper(*inbufptr++);
              curtokint *= i;
              if (ch <= '9')
                curtokint += ch - '0';
            ...
            if ((toupper(*inbufptr) == 'B' && i == 8) || (toupper(*inbufptr) == 'H' && i == 16))
              inbufptr++;
