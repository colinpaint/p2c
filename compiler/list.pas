...
begin
  ...
  if sharedPtr^.morefiles then
    ...
    while sharedPtr^.stringblkptr^[sharedPtr^.nextstringfile] <> 0 do
      ...
      sharedPtr^.filename[sharedPtr^.filename_length] := chr(sharedPtr^.stringfile^[sharedPtr^.nextstringfile]);
      getstringfile;
      end;
...
begin
  ...
  if sharedPtr^.fakelist then
    ...
    rewrite (sharedPtr^.listing, sharedPtr^.listname);
    }
    ...
    {
    printlisting (sharedPtr^.listing);
    }

    {
    close (sharedPtr^.listing);
    }
    end;
