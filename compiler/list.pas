...
{<<<}
procedure listSeekStringFile (n: integer {byte to access} );
...
begin
  ...
  if sharedPtr^.morefiles then
    begin
    listSeekStringFile (sharedPtr^.stringfilecount + curfileptr^.offset - 1);
...
{>>>}
{<<<}
procedure printlisting (var listFile: text);
...
{>>>}
  ...
  { Dump out all the characters in the buffer. }
    ...
    begin {wl_line}
      writeln (listFile, buffer: buffer_ctr);
      buffer_ctr := 0;
    end; {wl_line}
      ...
      case sourcestate of
        ...
        skippingblanks:
          if (ch <> ' ') and (ch <> chr(tabch)) then
            begin
            for i := 1 to filenamelen do sharedPtr^.filename[i] := ' ';
            if ch = '''' then
              begin
              sharedPtr^.filename_length := 0;
