...
{ forwards }
{<<<}
{ Walk a tree or subtree building a value.  See the forward body for further data }
procedure walkvalue (root: nodeindex; {root of tree to walk}
                     var key: keyindex; {resulting key}
                     targetkey: keyindex {target (0 if none)} ); forward;
{>>>}
{<<<}
...
begin
  ...
  if sharedPtr^.switcheverplus[tswitch1] and sharedPtr^.switcheverplus[details] then
    begin
...
{<<<  fp externs}
{ Compute:   z := x + y; If any error (overflow, NaN, etc), 'err' is set and z is not valid. }
procedure fpadd(x, y: realarray; var z: realarray; var err: boolean); external;

{ Compute:   z := x - y; Invert the sign of y and add.
  If any error (overflow, NaN, etc), 'err' is set and z is not valid. }
procedure fpsub(x, y: realarray; var z: realarray; var err: boolean); external;

{ Compute:   z := x * y; If any error (overflow, NaN, etc), 'err' is set and z is not valid. }
procedure fpmul(x, y: realarray; var z: realarray; var err: boolean); external;

{ Compute:   z := x / y; If any error (overflow, NaN, etc), 'err' is set and z is not valid. }
procedure fpdiv(x, y: realarray; var z: realarray; var err: boolean); external;

{ Compare x to y, return +1, 0, -1 for greater, equal, less. We assume that neither x nor y are infinities or NaN's. }
function fpcomp(x, y: realarray): integer; external;

{ Compute:   y := -x; }
procedure fpnegate(x: realarray; var y: realarray);  external;

{ Convert IEEE double occupying first 8 bytes of d to extended in x }
procedure doubletofp(d: realarray; var x: realarray); external;

{ Convert IEEE single occupying first 4 bytes of d to extended in x }
procedure singletofp(s: realarray; var x: realarray); external;

{ Convert possibly unsigned integer i to floating point. }
procedure inttofp(i: integer; isunsigned: boolean; var x: realarray); external;
...
procedure fptodouble(x: realarray; var d: realarray; var err: boolean; var inexact: boolean); external;

{ Convert floating point x to IEEE single format.
  If overflow or other error occurs, set err, and s is not valid.
  If no error, 'inexact' means precision was lost. }
procedure fptosingle(x: realarray; var s: realarray; var err: boolean; var inexact: boolean); external;

{ Convert floating point x to DEC double format.
  If overflow or other error occurs, set err, and d is not valid.
  If no error, 'inexact' means precision was lost. }
procedure fptodecdouble(x: realarray; var d: realarray; var err: boolean; var inexact: boolean); external;

{ Convert floating point x to DEC single format.
  If overflow or other error occurs, set err, and s is not valid.
  If no error, 'inexact' means precision was lost. }
procedure fptodecsingle(x: realarray; var s: realarray; var err: boolean; var inexact: boolean); external;

{ Convert floating point x to an integer i, possibly unsigned.
  If overflow or other error occurs, set err, and i is not valid.
  If no error, 'inexact' means precision was lost. }
procedure fptoint(x: realarray; var i: integer; tounsigned: boolean; var err: boolean; var inexact: boolean); external;

{ Return a positive infinith }
procedure fpinfinity(var x: realarray); external;
{>>>}
{<<<}
...
begin
  if debugtree then
    if sharedPtr^.switcheverplus[details] and conditionsmatch then
      begin
...
begin
  ...
  if debugtree and sharedPtr^.switcheverplus[test] then
    close (dumpFile);
