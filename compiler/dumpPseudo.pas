...
var
  ...
  { Get and unpack the next element in the pseudofile, leaving the result in "pseudobuff". }
    ...
    { Do the equivalent of a get on the pseudofile, treating the file as a file of bytes.
      The pseudo-file is actually tempfileone, and the next byte is accessed as tempfileone^[nextpseudofile] }
...
begin
  ...
  while (pseudobuff.op <> endpseudocode) and not eof(tempfileone) do
    begin
    if pseudobuff.key > highestkey then
      highestkey := pseudobuff.key;
    dumppseudo;
    getpseudobuff;
    end;
