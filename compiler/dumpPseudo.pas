...
var
  ...
  {<<<}
  procedure init;
  { Set up the nokeydata and oneoperand sets. The assignments in this procedure were lifted verbatim from initcode }
  ...
  { Get and unpack the next element in the pseudofile, leaving the result in "pseudobuff". }
    ...
    { Do the equivalent of a get on the pseudofile, treating the file as a file of bytes.
      The pseudo-file is actually tempfileone, and the next byte is accessed as pseudoSharedPtr^.pseudoFile^.block[nextpseudofile] }
    ...
    begin {getint}
      ...
      if fudge.int = hostfilelim then
        for j := 1 to hostintsize * hostfileunits do
          begin
          fudge.byte[j] := pseudoSharedPtr^.pseudoFile^.block[nextpseudofile].byte;
          gettempfile;
          end;
      getint := fudge.int;
    end {getint} ;
    ...
    with pseudoSharedPtr^.pseudobuff do
      ...
      if (op = blockentry) then
        ...
        case op of
          ...
          defunsforlitindex: write ('defunsforlitidx': 15);
      ...
      if dumping then
        writeln (',', len: 3, ',(', key: 3, ',', refcount: 3, ',', copycount: 3, ') ',
                 oprnds[1]: 5, ' ', oprnds[2]: 5, ' ', oprnds[3]: 5);
      end;
  end;
  {>>>}
...
begin
  ...
  while (pseudoSharedPtr^.pseudobuff.op <> endpseudocode) and not eof(pseudoSharedPtr^.pseudoFile) do
    begin
    if pseudoSharedPtr^.pseudobuff.key > highestkey then
      highestkey := pseudoSharedPtr^.pseudobuff.key;
    dumpPseudo;
    getPseudoBuff;
    end;
