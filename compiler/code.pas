...
{ Final code when diagnostics are generated
  ...
  begin {fixdiags}
    ...
    if sharedPtr^.switcheverplus[debugging] then junk := dump_externals;
...
{ Push a procedure descriptor onto the stack.  This consists of pushing
  ...
  begin {pshprocx}
    ...
    if sharedPtr^.switcheverplus[sharecode] then
      begin
      settempadcon(usercall, pseudoSharedPtr^.pseudoinst.oprnds[2], unknown);
      settempreg(long, autod, sp);
      gensimplemove(tempkey + 1, tempkey);
      end
    else if $pic and not mc68020 then
  ...
  ** UNIX **
  ...
  begin {callroutinex}
    ...
    if pseudoSharedPtr^.pseudoinst.oprnds[3] <= 0 then
      ...
      if sharedPtr^.switcheverplus[sharecode] then
        begin
        if sharedPtr^.proctable[pseudoSharedPtr^.pseudoinst.oprnds[1]].externallinkage then
          settempadcon(usercall, pseudoSharedPtr^.pseudoinst.oprnds[1], unknown)
        else
          settempadcon(usercall, pseudoSharedPtr^.pseudoinst.oprnds[1], impure_section);
        settempareg(getareg);
        gensimplemove(tempkey + 1, tempkey);
        keytable[tempkey].oprnd.m := relative;
