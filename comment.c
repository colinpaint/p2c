...
int commentvisible (Strlist* cmt) {
  ...
  return (cmt &&
          ...
          ((eatcomments != 1 && eatcomments != 2) || isembedcomment(cmt)));
  }
//}}}
...
void steal_comments (long olds, long news, int always) {

  Strlist* cmt;
  Strlist *cmtfirst = NULL;
  Strlist *cmtblank = NULL;
...
/* Grab a comment off the end of the current line */
  ...
  *cp2 = 0;
  if (kind == CMT_ONBEGIN || kind == CMT_ONEND) {
    ...
    if (!strcincmp (cp, "procedure", 9) && isspace (cp[9]) && isalpha (cp[10]))
      cp += 10;
    else if (!strcincmp (cp, "function", 8) && isspace (cp[8]) && isalpha (cp[9]))
      cp += 9;
